...
JSON_PATH = (
    "/iris/u/marcelto/reward_learning/infer_output/setup_table_iter3_open_cum_qwen_single/"
    "reward_model_2026-05-26_10-19-19_qwen_open_cum_j79627_step002000_score_demos_20.json"
)

# Single reward dimension produced by the "single" model
KEYS = [
    ("Overall quality", "Overall quality"),
]

# One accent color for the single curve (dark teal — readable on both white and
# the green/red event bands)
COLORS = ["#2F6E7E"]

SMOOTH_WIN = 15  # moving-average window for the smoothed line

# All event ranges are emphasized on the single "Overall quality" curve.
HIGHLIGHTS = {
    "Overall quality": [(70, 100), (150, 200), (200, 240), (270, 300), (300, 340), (340, 400)],
}

# Event annotations: (start, end, label, color, kind)
SUCCESS_COLOR = "#2D8A3F"
FAILURE_COLOR = "#A82A2A"
WARNING_COLOR = "#E07A1F"
EVENTS = [
    ...
    (300, 340, "WARNING:\ndisproportionate\nreward obtained",       WARNING_COLOR, "warning"),
    (340, 400, "WARNING:\ncutlery doesn't\nreceive many points",    WARNING_COLOR, "warning"),
]
...
fig.savefig("reward_curves_single_no_shades.pdf", bbox_inches="tight", facecolor="white")
fig.savefig("reward_curves_single_no_shades.png", dpi=200, bbox_inches="tight", facecolor="white")
...
for (a, b, label, ecolor, kind) in EVENTS:
    band_alpha = 0.20 if kind in ("failure", "warning") else 0.15
...
fig.savefig("reward_curves_single.pdf", bbox_inches="tight", facecolor="white")
fig.savefig("reward_curves_single.png", dpi=200, bbox_inches="tight", facecolor="white")
print("Saved reward_curves_single.{pdf,png} and reward_curves_single_no_shades.{pdf,png}")
