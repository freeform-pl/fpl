...
y-axis: time to success = first_success_step
...
for side in ("left", "right"):
    ...
    jitter = rng.uniform(-0.18, 0.18, size=len(times))
    ax.scatter(
        x_for[side] + jitter, times,
...
ax.set_ylabel("Time to success", fontsize=13)
...
fig.savefig("slow_fast_scatter_with_single_time.pdf", bbox_inches="tight", facecolor="white")
fig.savefig("slow_fast_scatter_with_single_time.png", dpi=200, bbox_inches="tight", facecolor="white")
print("Saved slow_fast_scatter_with_single_time.{pdf,png}")
