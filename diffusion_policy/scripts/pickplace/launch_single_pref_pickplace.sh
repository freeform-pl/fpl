...
#SBATCH --mem=32G
#SBATCH --gres=gpu:1
#SBATCH --job-name=pp4_single_pref
...
# Composite scalar reward = average of order + per-object placed (4) +
# per-object drop (4).
export PIPELINE_DIR="pipeline_output_pickplace_4obj_fixed_single_pref"
export WANDB_PROJECT="pickplace_4obj_fixed_single_pref"
...
# Single composite scalar reward: average of order + per-object placed + per-object drop.
export REWARD_AXES="composite(order_reward+bread_placed+can_placed+milk_placed+cereal_placed+bread_drop+can_drop+milk_drop+cereal_drop)"
...
export CONDITIONING_TARGETS="0.8;0.8;0.8;0.8;0.8"
